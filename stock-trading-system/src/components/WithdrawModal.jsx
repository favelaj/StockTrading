...
const WithdrawModal = ({ isWithdrawModalOpen, setIsWithdrawModalOpen }) => {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.error || "Server error processing withdrawal.";
    ...
    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="w-full max-w-md mx-auto p-4 bg-white dark:bg-zinc-800 shadow-md rounded space-y-3">
        ...
        <div className="flex space-x-2">
          <button
            className="w-full dark:text-black bg-slate-400 py-2 px-4 rounded hover:bg-slate-500 transition"
          ...
          <button
            className="w-full text-white dark:text-black bg-[#6362E7] py-2 px-4 rounded hover:bg-[#4747e9] transition"
